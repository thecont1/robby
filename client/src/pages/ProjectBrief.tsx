...
import { Apple, Braces, Coins, Download, ExternalLink, FileText, Package, Sparkles, SquareTerminal, Terminal } from "lucide-react";
...
# Linux: same pattern with sha256sum and troid-linux-x86_64.tar.gz

# Windows (PowerShell) — throws before unpacking on mismatch
...
if ((Get-FileHash .\\troid-windows-x86_64.zip -Algorithm SHA256).Hash.ToLower() -ne $expected) { throw "SHA-256 mismatch" }
Expand-Archive .\\troid-windows-x86_64.zip -DestinationPath .`}</pre></div><div className="brief-steps"><h3>Three steps to your first reverse</h3><ol><li><Download size={16} strokeWidth={2.2} aria-hidden="true" /><div><strong>Download and unpack.</strong> Grab the archive above, verify it against <code>SHA256SUMS.txt</code>, and unpack it somewhere on your <code>PATH</code> (or anywhere you can reach from a shell).</div></li><li><Terminal size={16} strokeWidth={2.2} aria-hidden="true" /><div><strong>Write a <code>.robby</code> recipe.</strong> A recipe is a few lines of plain text pointing at one JPEG. Point <code>base</code> at your photograph and decide the palette size and reverse mode:</div><pre>{`base("night-street.jpg")
palette(k: 8)
reverse(mode: "negative")
output(obverse: "night-street.jpg", reverse: "transient", manifest: "transient")`}</pre></li><li><Braces size={16} strokeWidth={2.2} aria-hidden="true" /><div><strong>Validate, then compile.</strong> <code>troid check</code> parses and validates the recipe; <code>troid compile</code> lowers it to the versioned JSON intermediate representation:</div><pre>{`troid check   recipe.robby
troid compile recipe.robby --out recipe.ir.json`}</pre></li></ol></div><p>The reverse itself is rendered against your JPEG by <code>troid render</code>, by the local HTTP server in this deck, or by the browser WASM build. The binary never stores a reverse on disk — the obverse and the recipe are the durable inputs; the reverse is regenerated per view, byte for byte.</p></Section></section></div><SiteFooter /></main>;
}
