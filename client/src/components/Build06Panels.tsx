...
function TraceSignatures({ item, credentialLabel, byteLength }: { item: GalleryItem; credentialLabel: string; byteLength: number }) {
  return <div className="trace-evidence">
    <div className="signature-label"><Fingerprint size={13} /> <span className="mono-label">Colour signature</span></div>
    <div className="palette-row" aria-label="Calculated palette signature">{item.palette.map(color => <span key={color} style={{ backgroundColor: color }} title={color} />)}</div>
    <dl><div><dt>credential_signature</dt><dd>{credentialLabel} · {item.credentialSignature.sourceSha256.slice(0, 14)}…</dd></div><div><dt>colour_signature</dt><dd>px:{item.colourSignature.pixelSha256.slice(0, 12)}… · pal:{item.colourSignature.paletteSha256.slice(0, 12)}…</dd></div><div><dt>script_input</dt><dd>{byteLength.toLocaleString()} UTF-8 bytes</dd></div></dl>
  </div>;
}
...
export function CompilationTraceModes({
  ...
  const credentialLabel = item.credentialSignature.status === "present" ? "C2PA PRESENT" : item.credentialSignature.status === "candidate" ? "C2PA CANDIDATE" : "C2PA ABSENT";
