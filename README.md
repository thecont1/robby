...
For `palette_grid`, the canonical reverse is still generated by Rust as a transient PNG and manifest. The gallery may present that same compiler result through a p5.js canvas: p5 receives only the Rust manifest’s palette and derived seed, then animates deterministic row/column shifts for the artwork. It is a presentation layer, not a second compiler or an alternative source of truth. Users who prefer a still image, including those who request reduced motion, see the canonical Rust PNG.
